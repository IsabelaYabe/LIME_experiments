...
def main():
  ...
  for Z in range(args.num_rounds):
    ...
    # Configurando o ShuffleSplit
    ss = ShuffleSplit(n_splits=1, test_size=0.2, random_state=42)

    # Obter índices de treino e teste
    for train_index, test_index in ss.split(dirty_train_vectors):
    ...
    cv_preds1 = cross_val_predict(c1, dirty_train_vectors[train_idx], train_labels[train_idx], cv=5)
    cv_preds2 = cross_val_predict(c2, dirty_train_vectors[train_idx], train_labels[train_idx], cv=5)
